...
mkdir -p $TARGET_DIR
TARGET_COMPILER=clang; eval "echo \"$(cat ./Dockerfile.mkl)\"" > $TARGET_DIR/Dockerfile
...
mkdir -p $TARGET_DIR
TARGET_COMPILER=clang; eval "echo \"$(cat ./Dockerfile.oss)\"" > $TARGET_DIR/Dockerfile
ALLGEBRA_TARGET=${TARGET_DIR}; eval "echo \"$(cat ./Makefile.in)\"" > $TARGET_DIR/Makefile

# cuda gcc mkl
TARGET_DIR=cuda${CUDA_MAJOR}_${CUDA_MINOR}/gcc${GCC_MAJOR}/mkl
mkdir -p $TARGET_DIR
TARGET_COMPILER=gcc; eval "echo \"$(cat ./Dockerfile.mkl)\"" > $TARGET_DIR/Dockerfile
ALLGEBRA_TARGET=${TARGET_DIR}; eval "echo \"$(cat ./Makefile.in)\"" > $TARGET_DIR/Makefile

# cuda gcc oss
TARGET_DIR=cuda${CUDA_MAJOR}_${CUDA_MINOR}/gcc${GCC_MAJOR}/oss
mkdir -p $TARGET_DIR
TARGET_COMPILER=gcc; eval "echo \"$(cat ./Dockerfile.oss)\"" > $TARGET_DIR/Dockerfile
ALLGEBRA_TARGET=${TARGET_DIR}; eval "echo \"$(cat ./Makefile.in)\"" > $TARGET_DIR/Makefile
...
mkdir -p $TARGET_DIR
eval "echo \"$(cat ./Dockerfile.cuda)\"" > Dockerfile
ALLGEBRA_TARGET=${TARGET_DIR}; eval "echo \"$(cat ./Makefile.in)\"" > $TARGET_DIR/Makefile
